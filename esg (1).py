...
Original file is located at
    https://colab.research.google.com/drive/1Gjj0AS_uKDBhGp4idHNUSEv_racUNCTL
...
abc.sentiment('Heavy rainfall has lead to heavy floods','esg_sentiment')
...
samples = [
    '10000 barrels of crude oil spilled in pacific ocean',
    'Bhopal gas leak affected hundred kilometer of dense populated area',
    'Amazon forest is being affected by the deforestation and pollution for more than 10 years',
    'somalia spilled in 863950 barrels per day of crude from Saudi Arabia',
    'In September 2016, the Consumer Financial Protection Bureau (CFPB) imposed a penalty',
    'This photo taken by Harrogate resident Adrian Davy on the morning of September 10, 2022 shows dead fish in Oak Beck after the pollution spillage'
    ]
...
data['Summary'] = data.News.progress_apply(lambda x : predictor.summerize(x))
data['ORG2'] = data.Summary.progress_apply(lambda x : ner_predict(x))
...
"""## Concluded, ongoing partially, concluded"""

import spacy

nlp = spacy.load("en_core_web_sm")
doc = nlp("The Boeing company makies silicon for Apple. The Boeing Company faced multiple investigations related to two crashes of Boeing 737 Max 8.")

for token in doc:
    print(token.text, token.lemma_, token.pos_, token.tag_, token.dep_,
            token.shape_, token.is_alpha, token.is_stop, token.head)

concluded =  """In May 2022, the Securities and Exchange Commission (SEC) imposed a penalty of USD 7 million against Wells Fargo Advisors, a broker-dealer and investment adviser unit of Wells Fargo & Company, for failing to timely file Suspicious Activity Reports (SARS) in violation of its anti-money laundering obligations. According SEC's findings, between April 2017 and October 2021, Wells Fargo Advisors to timely file at least 34 SARS. Wells Fargo Advisors also agreed to SEC's censure and cease and desist order.

UPDATES:

22 May 2022: SEC Charges Wells Fargo Advisors With Anti-Money Laundering Related Violations. (SEC)"""

concluded_list = concluded.split('. ')

concluded_list

check_ongoing_status(ongoing_list)

ongoing = """The Boeing company makies silicon for Apple. The Boeing Company faced multiple investigations related to two crashes of Boeing 737 Max 8."""

ongoing_list = ongoing.split('. ')
...
model = AutoModelForSeq2SeqLM.from_pretrained("sshleifer/distilbart-xsum-12-3")

inputs = tokenizer(ongoing, max_length=1024, return_tensors="pt", truncation=True)
...
lista = ['sdfksj','skjdfslk','ajkshfdhkjsa','jksdfkjsk']

pd.DataFrame(lista, columns=['News'])
